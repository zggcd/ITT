	 ...
	 var methods = {
		...
		*/
		_dateRange: function (field, rules, i, options) {
			...
			var splitFirstDate = options.firstOfGroup[0].value.split('/');
        	var splitSecondDate = options.secondOfGroup[0].value.split('/');
      
			if (methods._isDate(options.firstOfGroup[0].value) && methods._isDate(options.secondOfGroup[0].value)&&(new Date(splitFirstDate [2], splitFirstDate [1] - 1, splitFirstDate [0]) > new Date(splitSecondDate [2], splitSecondDate [1] - 1, splitSecondDate [0]))) {
				if(field.attr("id").indexOf("NgayCoHieuLuc")!=-1)
					return "Ngày hiệu lực phải trước ngày hết hiệu lực.";
				else if(field.attr("id").indexOf("NgayHetHieuLuc")!=-1)
