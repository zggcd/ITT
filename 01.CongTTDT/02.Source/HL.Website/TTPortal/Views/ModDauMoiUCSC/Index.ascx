...
<%
    ...
    var listEntity = ViewBag.Data as List<ModDauMoiUCSCEntity>;
%>
...
<script type="text/javascript">
    ...
    var HLController = 'ModDauMoiUCSC';
                ...
                <td nowrap="nowrap">
                   Chuyên mục : 
                    <select id="filter_menu" onchange="HLRedirect()" class="inputbox" size="1">
                         ...
                         <%= Utils.ShowDDLMenuByType("DauMoiUCSC", model.LangID, model.MenuID)%> 
                    </select>
                   Vị trí :
                   <select id="filter_state" onchange="HLRedirect()" class="inputbox" size="1">
                         <option value="0">(Tất cả)</option>
                         <%= Utils.ShowDDLByConfigkey("Mod.DauMoiUCSCState", model.State)%>
                    </select>
        ...
        <table class="adminlist" cellspacing="1">
            <thead>
                <tr>
                    ...
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Chuc vu", "ChucVu")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Dia chi", "DiaChi")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Dien thoai", "DienThoai")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Dien thoai d d", "DienThoaiDD")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Fax", "Fax")%>
                    </th>
            ...
            <tbody>
               <%for (int i = 0; listEntity != null && i < listEntity.Count; i++)
                ...
                <tr class="row<%= i%2 %>">
                    ...
                    <td align="center">
                       <%= listEntity[i].DienThoai%>
                    </td>
                    <td align="center">
                       <%= listEntity[i].DienThoaiDD%>
                    </td>
                    <td align="center">
                       <%= listEntity[i].Fax%>
                    </td>
