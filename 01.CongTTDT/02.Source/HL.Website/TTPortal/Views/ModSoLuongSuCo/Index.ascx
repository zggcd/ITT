...
<%
    ...
    var listEntity = ViewBag.Data as List<ModSoLuongSuCoEntity>;
%>
...
<script type="text/javascript">
    ...
    var HLController = 'ModSoLuongSuCo';
                ...
                <td nowrap="nowrap">
                   Chuyên mục : 
                    <select id="filter_menu" onchange="HLRedirect()" class="inputbox" size="1">
                         ...
                         <%= Utils.ShowDDLMenuByType("SoLuongSuCo", model.LangID, model.MenuID)%> 
                    </select>
                   Vị trí :
                   <select id="filter_state" onchange="HLRedirect()" class="inputbox" size="1">
                         <option value="0">(Tất cả)</option>
                         <%= Utils.ShowDDLByConfigkey("Mod.SoLuongSuCoState", model.State)%>
                    </select>
        ...
        <table class="adminlist" cellspacing="1">
            <thead>
                <tr>
                    ...
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Bao cao tong hop", "BaoCaoTongHopID")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                    ...
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("So luong", "SoLuong")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Tu xu ly", "TuXuLy")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                    ...
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("De nghi", "DeNghi")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Thiet hai uoc tinh", "ThietHaiUocTinh")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
            ...
            <tbody>
               <%for (int i = 0; listEntity != null && i < listEntity.Count; i++)
                ...
                <tr class="row<%= i%2 %>">
                    ...
                    <td align="center">
                       <%= GetName(listEntity[i].getBaoCaoTongHop()) %>
                    </td>
                    <td align="center">
                    ...
                    <td align="center">
                       <%= string.Format("{0:#,##0}", listEntity[i].SoLuong)%>
                    </td>
                    <td align="center">
                       <%= string.Format("{0:#,##0}", listEntity[i].TuXuLy)%>
                    </td>
                    <td align="center">
                       <%= string.Format("{0:#,##0}", listEntity[i].ToChucHoTro)%>
                    </td>
                    <td align="center">
                       <%= string.Format("{0:#,##0}", listEntity[i].ToChucNuocNgoaiHoTro)%>
                    </td>
                    <td align="center">
                       <%= string.Format("{0:#,##0}", listEntity[i].DeNghi)%>
                    </td>
                    <td align="center">
                       <%= string.Format("{0:#,##0}", listEntity[i].ThietHaiUocTinh)%>
                    </td>
                    <td align="center">
