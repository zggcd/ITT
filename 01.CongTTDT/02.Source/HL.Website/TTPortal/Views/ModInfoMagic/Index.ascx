...
<%
    ...
    var listEntity = ViewBag.Data as List<ModInfoMagicEntity>;
%>
                ...
                <td nowrap="nowrap">
                   Chuyên mục : 
                    <select id="filter_menu" onchange="HLRedirect()" class="inputbox" size="1">
                         ...
                         <%= Utils.ShowDDLMenuByType("InfoMagic", model.LangID, model.MenuID)%> 
                    </select>
                   Vị trí :
                   <select id="filter_state" onchange="HLRedirect()" class="inputbox" size="1">
                         <option value="0">(Tất cả)</option>
                         <%= Utils.ShowDDLByConfigkey("Mod.InfoMagicState", model.State)%>
                    </select>
        ...
        <table class="adminlist" cellspacing="1">
            <thead>
                <tr>
                    ...
                    <th width="1%" nowrap="nowrap">
                        <%= GetSortLink("Bao cao ban dau su co", "BaoCaoBanDauSuCoID")%>
                    </th>
                    <th width="1%" nowrap="nowrap">
            ...
            <tbody>
               <%for (int i = 0; listEntity != null && i < listEntity.Count; i++)
                ...
                <tr class="row<%= i%2 %>">
                    ...
                    <td align="center">
                       <%= GetName(listEntity[i].getBaoCaoBanDauSuCo()) %>
                    </td>
                    <td align="center">
