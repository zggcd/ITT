...
{
    public class FormBaoCaoPhuongAnSuCoController : CPController
    {
        public FormBaoCaoPhuongAnSuCoController()
        {
            //khoi tao Service
            DataService = ModBaoCaoPhuongAnSuCoService.Instance;
            CheckPermissions = true;
        }

        public void ActionIndex(FormBaoCaoPhuongAnSuCoModel model)
        {
            // sap xep tu dong
            string orderBy = AutoSort(model.Sort);

            // tao danh sach
            var dbQuery = ModBaoCaoPhuongAnSuCoService.Instance.CreateQuery()
        ...
        public void ActionAdd(ModBaoCaoPhuongAnSuCoEntity entity, MAppend append, string endCode)
        {
            if (entity.ID == 0) 
            {
                entity = new ModBaoCaoPhuongAnSuCoEntity();
            ...
            else
            {
                ...
                if (isValid == true)
                {
                    ...
                    if (sc != null)
                    {
                        string code = "BCDBSC" + ModBaoCaoPhuongAnSuCoService.Instance.GetMaxID();
                        entity.Name = code;
        ...
        private ModBaoCaoPhuongAnSuCoEntity entity = null;
    ...
    public class FormBaoCaoPhuongAnSuCoModel : DefaultModel
    {
