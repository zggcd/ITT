...
{
    public class FormBaoCaoHoTroPhoiHopSuCoController : CPController
    {
        public FormBaoCaoHoTroPhoiHopSuCoController()
        {
            //khoi tao Service
            DataService = ModBaoCaoHoTroPhoiHopSuCoService.Instance;
            CheckPermissions = true;
        }

        public void ActionIndex(FormBaoCaoHoTroPhoiHopSuCoModel model)
        {
            // sap xep tu dong
            string orderBy = AutoSort(model.Sort);

            // tao danh sach
            var dbQuery = ModBaoCaoHoTroPhoiHopSuCoService.Instance.CreateQuery()
        ...
        public void ActionAdd(ModBaoCaoHoTroPhoiHopSuCoEntity entity, MAppend append, string endCode)
        {
            if (entity.ID == 0) 
            {
                entity = new ModBaoCaoHoTroPhoiHopSuCoEntity();
            ...
            else
            {
                ...
                if (isValid == true)
                {
                    ...
                    if (sc != null)
                    {
                        string code = "BCDBSC" + ModBaoCaoHoTroPhoiHopSuCoService.Instance.GetMaxID();
                        entity.Name = code;
        ...
        private ModBaoCaoHoTroPhoiHopSuCoEntity entity = null;

        private bool ValidBCPhuongAn(ref ModBaoCaoHoTroPhoiHopSuCoEntity entity, MAppend append, string endCode)
        {
    ...
    public class FormBaoCaoHoTroPhoiHopSuCoModel : DefaultModel
    {
