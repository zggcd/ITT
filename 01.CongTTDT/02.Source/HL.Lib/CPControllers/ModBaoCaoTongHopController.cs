...
namespace HL.Lib.CPControllers
{
    public class ModBaoCaoTongHopController : CPController
    {
        ...
        public void ActionSave(ModBaoCaoTongHopModel model)
        {
            if (ValidSave(model))
               SaveRedirect();
        }

        public void ActionApply(ModBaoCaoTongHopModel model)
        {
            if (ValidSave(model))
               ApplyRedirect(model.RecordID, entity.ID);
        }

        public void ActionSaveNew(ModBaoCaoTongHopModel model)
        {
            if (ValidSave(model))
        ...
        private bool ValidSave(ModBaoCaoTongHopModel model)
        {
            ...
            if (CPViewPage.Message.ListMessage.Count == 0)
            {
                ...
                //save
                ModBaoCaoTongHopService.Instance.Save(entity);
